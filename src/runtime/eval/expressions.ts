import type {
  ArrayLiteral,
  AssignmentExpr,
  BinaryExpr,
  CallExpr,
  Identifier,
  MemberExpr,
  ObjectLiteral,
  UnaryExpr,
  LambdaExpr,
  AwaitExpr,
} from "../../frontend/ast";
import Environment from "../environment";
import { evaluate } from "../interpreter";
import { unwrapAwait } from "../../lib/Utils";
import { ReturnSignal } from "./statements";
import {
  MK_ARRAY,
  MK_NULL,
  MK_NUMBER,
  type ArrayValue,
  type BooleanValue,
  type FunctionValue,
  type NativeFnValue,
  type NumberValue,
  type ObjectValue,
  type RuntimeValue,
  type StringValue,
  MK_PROMISE,
  type PromiseValue,
  MK_AWAIT_RESULT,
  type AwaitResultValue,
  MK_STRING,
} from "../values";

function isTruthy(val: RuntimeValue): boolean {
  const unwrapped = unwrapAwait(val);
  if (unwrapped.type === "boolean") return (unwrapped as BooleanValue).value;
  if (unwrapped.type === "number")
    return (unwrapped as NumberValue).value !== 0;
  if (unwrapped.type === "null") return false;
  return true;
}

export async function evaluateUnaryExpr(
  unary: UnaryExpr,
  env: Environment,
): Promise<RuntimeValue> {
  const arg = unwrapAwait(await evaluate(unary.argument, env));

  if (unary.operator === "!") {
    return { type: "boolean", value: !isTruthy(arg) } as any;
  }

  if (unary.operator === "-") {
    if (arg.type !== "number")
      throw "Cannot apply '-' operator to non-number type.";
    return MK_NUMBER(-(arg as NumberValue).value);
  }

  return MK_NULL();
}

function eval_numeric_binary_expr(
  operator: string,
  LHS: NumberValue,
  RHS: NumberValue,
): RuntimeValue {
  let result = 0;

  switch (operator) {
    case "+":
      result = LHS.value + RHS.value;
      break;
    case "-":
      result = LHS.value - RHS.value;
      break;
    case "*":
      result = LHS.value * RHS.value;
      break;
    case "/":
      result = LHS.value / RHS.value;
      break;
    case "%":
      result = LHS.value % RHS.value;
      break;
    default:
      console.error("This operator has not yet been setup for interpretation");
      process.exit(0);
  }

  return {
    type: "number",
    value: result,
  } as NumberValue;
}

export async function evaluateBinaryExpr(
  binop: BinaryExpr,
  env: Environment,
): Promise<RuntimeValue> {
  const LHS = unwrapAwait(await evaluate(binop.left, env));

  // Short-circuiting for logical operators
  if (binop.operator === "&&") {
    if (!isTruthy(LHS)) return LHS;
    return unwrapAwait(await evaluate(binop.right, env));
  }
  if (binop.operator === "||") {
    if (isTruthy(LHS)) return LHS;
    return unwrapAwait(await evaluate(binop.right, env));
  }

  const RHS = unwrapAwait(await evaluate(binop.right, env));

  // 1. Numeric Binary Expressions
  if (LHS.type == "number" && RHS.type == "number") {
    const l = LHS as NumberValue;
    const r = RHS as NumberValue;

    if (binop.operator === "==")
      return { type: "boolean", value: l.value === r.value } as any;
    if (binop.operator === "!=")
      return { type: "boolean", value: l.value !== r.value } as any;
    if (binop.operator === "<")
      return { type: "boolean", value: l.value < r.value } as any;
    if (binop.operator === ">")
      return { type: "boolean", value: l.value > r.value } as any;
    if (binop.operator === "<=")
      return { type: "boolean", value: l.value <= r.value } as any;
    if (binop.operator === ">=")
      return { type: "boolean", value: l.value >= r.value } as any;

    return eval_numeric_binary_expr(binop.operator, l, r);
  }

  // 2. Equality check for non-numbers (Booleans, Strings, Null)
  if (binop.operator === "==") {
    return {
      type: "boolean",
      value: (LHS as any).value === (RHS as any).value,
    } as any;
  }
  if (binop.operator === "!=") {
    return {
      type: "boolean",
      value: (LHS as any).value !== (RHS as any).value,
    } as any;
  }

  // 3. String Concatenation
  if (
    binop.operator === "+" &&
    (LHS.type === "string" || RHS.type === "string")
  ) {
    const lValue =
      LHS.type === "string" ? (LHS as StringValue).value : (LHS as any).value;
    const rValue =
      RHS.type === "string" ? (RHS as StringValue).value : (RHS as any).value;
    return {
      type: "string",
      value: String(lValue) + String(rValue),
    } as StringValue;
  }

  return MK_NULL();
}

export function evaluateIdentifier(
  ident: Identifier,
  env: Environment,
): RuntimeValue {
  return env.lookupVar(ident.symbol);
}

export async function evaluateMemberExpr(
  member: MemberExpr,
  env: Environment,
): Promise<RuntimeValue> {
  const object = unwrapAwait(await evaluate(member.object, env));

  if (object.type === "object") {
    const obj = object as ObjectValue;
    const property = member.computed
      ? unwrapAwait(await evaluate(member.property, env))
      : ({
          type: "string",
          value: (member.property as Identifier).symbol,
        } as any);

    return obj.properties.get((property as any).value) || MK_NULL();
  }

  if (object.type === "array") {
    const arr = object as ArrayValue;
    if (!member.computed) {
      // Support .length for arrays?
      const prop = (member.property as Identifier).symbol;
      if (prop === "length") return MK_NUMBER(arr.elements.length);
      throw `Array does not have property: ${prop}`;
    }

    const index = unwrapAwait(await evaluate(member.property, env));
    if (index.type !== "number") {
      throw `Array index must be a number, got ${index.type}`;
    }

    const i = (index as NumberValue).value;
    return arr.elements[i] || MK_NULL();
  }

  throw `Cannot use member expression on type: ${object.type}`;
}

export async function evaluateArrayLiteral(
  arr: ArrayLiteral,
  env: Environment,
): Promise<RuntimeValue> {
  const elements = [];
  for (const el of arr.elements) {
    elements.push(unwrapAwait(await evaluate(el, env)));
  }
  return MK_ARRAY(elements);
}

export async function evalObjectExpr(
  obj: ObjectLiteral,
  env: Environment,
): Promise<RuntimeValue> {
  const object: ObjectValue = {
    type: "object",
    properties: new Map<string, RuntimeValue>(),
  };

  for (const { key, value } of obj.properties) {
    const runtimeValue =
      value == null
        ? env.lookupVar(key)
        : unwrapAwait(await evaluate(value, env));

    object.properties.set(key, runtimeValue);
  }

  return object;
}

export async function evaluateCallExpr(
  expr: CallExpr,
  env: Environment,
): Promise<RuntimeValue> {
  const args: any[] = [];
  for (const arg of expr.args) {
    args.push(unwrapAwait(await evaluate(arg, env)));
  }

  const caller = unwrapAwait(await evaluate(expr.caller, env));

  if (caller.type === "native-fn") {
    // Native functions might be async or sync
    const result = (caller as NativeFnValue).call(args, env);
    if (result && result.type === "promise") {
      if (!(global as any).pendingPromises)
        (global as any).pendingPromises = [];
      (global as any).pendingPromises.push((result as any).promise);
    }
    return result;
  }

  if (caller.type === "function") {
    const func = caller as FunctionValue;

    const executeFunction = async () => {
      const scope = new Environment(func.declarationEnv, func.async);

      // Create the variables for the parameters
      for (let i = 0; i < func.parameters.length; i++) {
        const varname = func.parameters[i]!;
        scope.declareVar(varname, args[i] ?? MK_NULL(), false);
      }

      let results: RuntimeValue = MK_NULL();
      try {
        for (const stmt of func.body) {
          results = unwrapAwait(await evaluate(stmt, scope));
        }
      } catch (e) {
        if (e instanceof ReturnSignal) {
          return e.value;
        }
        throw e;
      }
      return results;
    };

    if (func.async) {
      const promise = executeFunction();
      // Track pending promises to ensure program waits for them
      if (!(global as any).pendingPromises)
        (global as any).pendingPromises = [];
      (global as any).pendingPromises.push(promise);
      return MK_PROMISE(promise);
    } else {
      return await executeFunction();
    }
  }

  throw `Cannot call value that is not a function: ${JSON.stringify(caller)}`;
}

export async function evaluateAssignment(
  node: AssignmentExpr,
  env: Environment,
): Promise<RuntimeValue> {
  let value = await evaluate(node.value, env);

  // Special handling for AwaitResult
  if (value.type === "await-result") {
    const awaitRes = value as AwaitResultValue;
    if (node.identifiers) {
      // Destructuring: (r, e) = await ...
      value = MK_ARRAY([awaitRes.result, awaitRes.error]);
    } else {
      // Single var: r = await ...
      if (awaitRes.error.type !== "null") {
        throw (awaitRes.error as any).value;
      }
      value = awaitRes.result;
    }
  }

  if (node.identifiers) {
    if (value.type !== "array") {
      throw `Cannot destructure non-array value. Got ${value.type}`;
    }
    const arr = value as ArrayValue;
    for (let i = 0; i < node.identifiers.length; i++) {
      env.assignVar(node.identifiers[i]!, arr.elements[i] || MK_NULL());
    }
    return value;
  }

  if (node.assignee.kind === "Identifier") {
    const varname = (node.assignee as Identifier).symbol;
    return env.assignVar(varname, value);
  }

  if (node.assignee.kind === "MemberExpr") {
    const member = node.assignee as MemberExpr;
    const object = await evaluate(member.object, env);

    if (object.type === "object") {
      const obj = object as ObjectValue;
      const property = member.computed
        ? await evaluate(member.property, env)
        : ({
            type: "string",
            value: (member.property as Identifier).symbol,
          } as any);

      obj.properties.set((property as any).value, value);
      return value;
    }

    if (object.type === "array") {
      const arr = object as ArrayValue;
      if (!member.computed) {
        throw "Cannot assign to a non-computed property of an array.";
      }

      const index = await evaluate(member.property, env);
      if (index.type !== "number") {
        throw `Array index must be a number, got ${index.type}`;
      }

      arr.elements[(index as NumberValue).value] = value;
      return value;
    }

    throw `Cannot assign to a property of type: ${object.type}`;
  }

  throw `Invalid LHS inside assignment expression: ${JSON.stringify(node.assignee)}`;
}

export function evaluateLambdaExpr(
  lambda: LambdaExpr,
  env: Environment,
): RuntimeValue {
  const func: FunctionValue = {
    type: "function",
    name: "anonymous",
    parameters: lambda.parameters,
    body: lambda.body,
    async: lambda.async,
    declarationEnv: env,
  };

  return func;
}

export async function evaluateAwaitExpr(
  node: AwaitExpr,
  env: Environment,
): Promise<RuntimeValue> {
  if (!env.isAsyncContext) {
    throw "await can only be used inside an async function.";
  }
  const val = await evaluate(node.argument, env);

  if (val.type !== "promise") {
    // If it's not a promise, we return it in a success tuple (val, null)
    return MK_AWAIT_RESULT(val, MK_NULL());
  }

  const promiseVal = val as PromiseValue;
  try {
    const result = await promiseVal.promise;

    // Remove from pendingPromises since it's now handled
    if ((global as any).pendingPromises) {
      const index = (global as any).pendingPromises.indexOf(promiseVal.promise);
      if (index > -1) {
        (global as any).pendingPromises.splice(index, 1);
      }
    }

    return MK_AWAIT_RESULT(result, MK_NULL());
  } catch (error) {
    // Even if it failed, it's "handled" now via the tuple
    if ((global as any).pendingPromises) {
      const index = (global as any).pendingPromises.indexOf(promiseVal.promise);
      if (index > -1) {
        (global as any).pendingPromises.splice(index, 1);
      }
    }
    return MK_AWAIT_RESULT(MK_NULL(), MK_STRING(String(error)));
  }
}
